...
extract_daily_ima = function(ref_date){
  ...
  entra_na_pagina_response <- httr::GET(entra_na_pagina_url)
  entra_na_pagina_cookies <- httr::cookies(entra_na_pagina_response)

  cookies <- entra_na_pagina_cookies$value %>% stats::setNames(entra_na_pagina_cookies$name)
  ...
  clica_no_download_html <- httr::POST(
...
extract_batch_usdbrl = function(ini_date,end_date){
  ...
  pega_tabela_response <- httr::POST(
    pega_tabela_url,
    body = dados,
    httr::add_headers("User-Agent" = "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2661.75 Safari/537.36",
                "X-Requested-With" = "XMLHttpRequest"),
    httr::accept("text/plain, */*; q=0.01"),
    encode = "form"
  )
  ...
  df_usd[,1] <- df_usd[,1] %>% gsub("\\.","/",.) %>% as.Date(.,"%d/%m/%Y")
  df_usd[,2] <- df_usd[,2] %>% gsub(",","\\.",.) %>% as.numeric()
...
extract_batch_ibov = function(ini_date, end_date){
  ...
  df_out[,1] <- df_out[,1] %>% gsub("\\.","/",.) %>% as.Date(.,"%d/%m/%Y")
  df_out[,2] <- df_out[,2] %>% gsub("\\.","",.) %>% gsub(",","\\.",.) %>% as.numeric()
